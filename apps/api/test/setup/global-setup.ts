...
const ADMIN =
  process.env.TEST_DATABASE_ADMIN_URL ?? 'postgres://fa_owner:fa_owner@localhost:5432/postgres';
...
export default async function globalSetup(): Promise<void> {
  ...
  execSync('npx tsx node_modules/knex/bin/cli.js --knexfile knexfile.ts migrate:latest', {
    cwd: dbPkg,
    env: {
      ...process.env,
  ...
  execSync('npx tsx node_modules/knex/bin/cli.js --knexfile knexfile.ts seed:run', {
    cwd: dbPkg,
    env: {
      ...process.env,
      DATABASE_ADMIN_URL: adminTestUrl,
  ...
  process.env.TEST_DATABASE_URL =
    process.env.TEST_DATABASE_URL ?? `postgres://fa_runtime:fa_runtime@localhost:5432/${TEST_DB}`;
