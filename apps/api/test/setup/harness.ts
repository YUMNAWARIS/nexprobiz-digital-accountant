...
import { createContainer, createInfra, type Container } from '@/composition-root';
...
export async function createHarness(): Promise<Harness> {
  ...
  const storage = new LocalDocumentStorage(mkdtempSync(path.join(os.tmpdir(), 'fa-test-storage-')));
  ...
  return {
    ...
    async truncateAll() {
      ...
      await adminDb.raw('ALTER TABLE audit_events DISABLE TRIGGER audit_events_no_truncate');
      await adminDb.raw(`TRUNCATE ${tables.join(', ')} CASCADE`);
      await adminDb.raw('ALTER TABLE audit_events ENABLE TRIGGER audit_events_no_truncate');
